...
可以直接执行程序交互式输入样本数据（注意要输入 Ctrl+Z 结束输入）， ::
...
def main(infile, outfile):
    ...
    # 输出结果
    outfile.write('%d %d %s'%(tower_position[0], tower_position[1], connected and 'Yes' or 'No'))

if __name__ == '__main__':
    import sys, optparse
    parser = optparse.OptionParser(usage='%prog [-f] [-o] [-t]', version='%prog 1.0')
    parser.add_option("-f", "--file",
            help="input file, default to stdin", default=sys.stdin)
