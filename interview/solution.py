...
TODO: 使用 c 重新遍历地图部分 _traverse.c ，提供核心算法的执行性能。
...
def get_next_positions(position, direction):
    ...
    # 改成优先当前方向，并按你时针方向进行遍历
    base = [(x, y+1), (x-1, y), (x, y-1), (x+1,y)] # 右上左下
    result_map = [
        [base[3],base[2],base[1],base[0]], # 右
        [base[2],base[1],base[0],base[3]], # 下
        [base[1],base[0],base[3],base[2]], # 左
        [base[0],base[3],base[2],base[1]]  # 上
    ]
    '''
    result_map = [
        [base[0],base[1],base[2],base[3]], # 右
        [base[1],base[2],base[3],base[0]], # 下
        [base[2],base[3],base[0],base[1]], # 左
        [base[3],base[0],base[1],base[2]]  # 上
    ]
    '''
...
def traverse(graph, size, start, target):
    ...
    while visit_queue:
        ...
        if last_position:
            ...
            current_direction = RIGHT # 初始方向向右
        visited[position] = 1
